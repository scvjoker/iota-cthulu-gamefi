...
const LOADING_MSGS = {
    zh: [
        "正在聯絡人資部門...", "正在偽造加班紀錄...", "正在與不可名狀之物連線...",
        "老闆正在盯著你的螢幕...", "正在計算精神賠償...", "正在清空瀏覽器紀錄...",
        "正在將靈魂上傳至 IOTA 網絡...", "正在躲避辦公室的黑影..."
    ],
    en: [
...
function AdventureOverlay({ logs, onClose, outcome, lang }: { logs: string[], onClose: (ended: boolean, outcome: string) => void, outcome: string, lang: Lang }) {
    ...
    // 🔥 修正判定：使用更寬鬆的字串比對確保觸發結束畫面
    const lowerOutcome = outcome.toLowerCase();
    const isFinalEnding = lowerOutcome.includes("madness") || 
                         lowerOutcome.includes("escaped") || 
                         lowerOutcome.includes("resigned");
            ...
            <div className={`max-w-2xl w-full h-[80vh] flex flex-col gap-4 border-x border-slate-800 bg-slate-950/50 transition-all duration-500
                ...
                <div className={`p-4 border-b border-slate-800 ${isMadness ? "bg-red-950/50" : "bg-slate-900"}`}>
                    <h2 className={`text-xl font-mono flex items-center gap-2 ${isMadness ? "text-red-500" : "text-green-500"}`}>
                        <span className="animate-pulse">●</span> {isMadness ? "FATAL ERROR: MENTAL COLLAPSE" : "TRPG LOG"}
                    </h2>
                </div>
                ...
                {isFinished && (
                    <div className={`p-6 border-t border-slate-800 flex justify-center ${isMadness ? "bg-red-950/50" : "bg-slate-900"}`}>
                        <Button onClick={() => onClose(isFinalEnding, outcome)} className={`w-full max-w-sm font-bold text-lg h-12 transition-colors ${isMadness ? "bg-red-600 text-white hover:bg-red-500" : "bg-slate-100 text-black hover:bg-white"}`}>
                            {isMadness ? "EMBRACE THE DARKNESS" : (outcome === "Looping" ? "🔄 Continue" : "🚪 Close")}
                        </Button>
                    </div>
                )}
            </div>
...
export function GameDashboard({ lang }: { lang: Lang }) {
  ...
  const [playWhisper] = useSound('/sounds/whisper.mp3', { volume: 0.8 });
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      if (isBroken) { document.title = "🆘 HELP ME HELP ME HELP ME"; } 
      else { document.title = lang === 'zh' ? "IOTA 社畜調查員" : "IOTA Investigator"; }
    }
  }, [isBroken, lang]);
  ...
  useEffect(() => {
    if (loading) {
        const msgs = (LOADING_MSGS as any)[lang] || LOADING_MSGS.en;
  ...
  const executeScenarioTx = async (tx: Transaction) => { 
      ...
      signAndExecuteTransaction({ transaction: tx as any, options: { showEffects: true, showEvents: true } }, { 
          onSuccess: async (result: any) => { 
              ...
              const event = events.find((e: any) => e.type.includes("ScenarioEvent")); 
              if (event) { 
  ...
  const resign = () => {
      ...
      signAndExecuteTransaction({ transaction: tx as any, options: { showEffects: true, showEvents: true } }, {
          onSuccess: () => { 
              playStamp(); 
              setLoading(false); 
              toast.success(lang === 'zh' ? "離職手續已完成" : "Resignation Accepted");
  ...
  return (
      <>
          ...
          {showIntro && <CinematicText lines={(t as any).intro_lines} onComplete={() => setShowIntro(false)} />}
          {outroType !== "none" && <CinematicText lines={outroType==="madness"?(t as any).outro_madness:(t as any).outro_escaped} onComplete={() => setOutroType("none")} isMadness={outroType==="madness"} />}
          ...
          {adventureData && (
            <AdventureOverlay 
                ...
                lang={lang} 
                onClose={(ended: boolean, outcomeStr: string) => { 
                    setAdventureData(null); 
                    fetchData(); 
                    // 🔥 修正：更精確地捕捉結束狀態並設置 Outro
                    if (ended) {
                        const lowOut = outcomeStr.toLowerCase();
                        if (lowOut.includes("madness") || lowOut.includes("mad")) {
                            setOutroType("madness");
                        } else if (lowOut.includes("escaped") || lowOut.includes("escape")) {
                            setOutroType("escaped");
                        }
                    }
                }} 
            />
          ...
          <div className="max-w-6xl mx-auto mt-8 px-4 pb-20 animate-in fade-in zoom-in-95 duration-500 relative z-20">
              <div className="text-center mb-8">
                <h1 className={`text-4xl md:text-5xl font-bold text-transparent bg-clip-text bg-linear-to-r from-red-500 to-indigo-600 font-serif tracking-widest uppercase mb-2 ${isBroken ? "animate-pulse" : ""}`}>
                    {(t as any).title}
                </h1>
                <p className="text-slate-500 text-sm tracking-widest">{(t as any).subtitle}</p>
              </div>
              <SimulationView 
                  account={account} investigators={investigators} memorials={memorials} 
                  currentInvestigator={currentInvestigator} loading={loading} t={t}
                  mint={mint} singleSanityCheck={singleSanityCheck} batchSanityCheck={batchSanityCheck} 
                  playQuick={playQuick} playManualStart={playManualStart} playManualAction={playManualAction}
                  resign={resign} setSelectedId={setSelectedId} 
                  onViewCert={(sbt:any) => { playClick(); setViewingCert(sbt); }} 
                  getImageUrl={getImageUrl} lang={lang}
              />
              <button onClick={triggerEasterEgg} className="fixed bottom-4 right-4 text-slate-800 hover:text-red-900 transition-colors opacity-20 hover:opacity-100 z-50" title="System Maintenance">
                <Phone size={16} />
              </button>
